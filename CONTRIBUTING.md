...
Edit dependency constraints only in `pyproject.toml`; `uv.lock` is generated by
the required uv 0.12.5 version and must never be hand-edited. Follow
[`docs/dependency_management.md`](docs/dependency_management.md) for targeted
updates, graph review, clean-install comparison, and security-audit evidence.
Do not add another requirements or constraints file.
