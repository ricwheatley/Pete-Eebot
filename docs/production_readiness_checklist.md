...
| [ ] Post-deploy smoke checks from `docs/runtime_deploy_runbook.md` pass: CLI status, systemd active, sync dry run/short run, Telegram listener if enabled, `/healthz`, `/readyz`, `/api/v1/status`, and `/api/v1/metrics`. | Blocker | Smoke-check transcript or summary. |
...
| [ ] `/readyz` is either restricted to trusted probe callers by the proxy or accepted as an unauthenticated detailed dependency endpoint by explicit risk signoff. | High | Proxy rule or risk record. |
...
| [ ] `/readyz?timeout=5` returns dependency readiness and fails with `503` when a required dependency is unhealthy. | High | Healthy result plus known-failure or staging negative test. |
...
| [ ] Operator triage path works without shell access where intended: `/console/status`, `/api/v1/logs?lines=200`, `/console/operations`, and `/console/trends`. | Medium | Browser/API check summary. |
...
| [ ] API key/session/security changes include a recovery path for locked-out operators. | High | Break-glass note. |
