...
| POST | `/console/operations/generate-plan` | Command | `operator`/`owner` browser session + CSRF + typed confirmation | `pete_e/api_routes/web.py` | Creates a durable `plan` job that starts `pete plan`. The fixed, read-only duration is 4 weeks; unsupported submitted values are rejected before job creation. Confirmation phrase: `GENERATE PLAN`. |
...
| POST | `/console/operations/preview-message` | Command | `operator`/`owner` browser session + CSRF | `pete_e/api_routes/web.py` | Queues an application callback that builds and stores one `summary`, `trainer`, or `plan` preview without sending it. |
