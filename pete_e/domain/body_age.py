...
def _extract_body_age_value(row: Dict[str, Any]) -> Optional[float]:
    ...
    value = extract_body_age_value(cast(BodyAgeHistoryRow, row))
...
def get_body_age_trend(dal: Any, target_date: Optional[date] = None) -> BodyAgeTrend:
    ...
    reader: BodyAgeHistoryReader = LegacyBodyAgeHistoryReader(dal)
    rows = reader.read_body_age_history(start, target_date)
    return analyze_body_age_trend(rows, target_date)
